...
class Patient(object):
    ...
    def update(self):
        ...
        for e in self.viruses:          
            if  (not e.doesClear()):
                updated.append(e)
        self.viruses = updated[:]
        ...
        for e in self.viruses:            
            ...
            try: 
                updated.append(e.reproduce((len(updated)*1.0/self.getMaxPop())))              
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        result.append([])
        p = Patient(infection, maxPop)
        for period in range(300):
            p.update()
            (result[trial]).append(p.getTotalPop())
            
    final = []
    for iteration in range(len(result[0])):
        total = 0
        for item in range(len(result)):
            total += 1.0*result[item][iteration]
        final.append(total/len(result))
        ...
        if (random.random() < self.getMaxBirthProb()*(1 - popDensity)):
            ...
            for drug in tmp:
                if (random.random() < self.mutProb):
                    if tmp.get(drug) == False:
                        tmp[drug] = True
                    else:
                        tmp[drug] = False
            return ResistantVirus(self.getMaxBirthProb(), self.getClearProb(), tmp, self.getMutProb())
        else:
            raise NoChildException
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for e in drugResist:
            for virus in self.viruses:
                if self.viruses[virus].resistances[e] == False:
                    result.remove(virus)
    ...
    def update(self):
        ...
        self.getResistPop(self.getPrescriptions)
